import {
  Conversation,
  Avatar,
  ConversationList
} from '@chatscope/chat-ui-kit-react';

import useUser from '../../hooks/useUser'

const MakeChat = (props) => {
  const { users, onSelectHandler, select, onSubmitHandler } = useUser();
  const defaultProfile = "data:image/png;base64,";
  
  const onCreateHandler = async () => {
    await onSubmitHandler()
    props.history.replace('/chats');
  }

  return (
    <div style={{
      height: window.innerHeight
    }}>
      {/* <Search placeholder="Search..." /> */}
      <ConversationList>
        {users.map((user, index) => (
          <Conversation name={user.name} info="message" unreadDot={select[index]} onClick={() => onSelectHandler(index)}>
            <Avatar src={user.profileImage || defaultProfile} name={user.name} />
          </Conversation>
        ))}
      </ConversationList>

      <button onClick={onCreateHandler}>submit</button>
    </div>
  );
}

export default MakeChat;